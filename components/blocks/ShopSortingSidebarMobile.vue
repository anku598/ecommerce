    ...
    <div class="mobile-filter">
      <div class="bar-header">
        ...
        >
          ...
          <!-- <div
            class="close-icon"
            :class="{ isActive: filterCollapseMobile === true }"
          >
      ...
      <div class="filter-sidebar-mobile" v-if="isSidebarFilterShow">
        ...
        <div class="sidebar-body">
          ...
          <div class="sort-item-mobile">
            ...
            <CategoryFilter
              label="Collection"
              v-if="
                pageType !== 'collection' && filterData.collections.length > 0
              "
              :is-node="false"
              :filter-data="filterData.collections"
              class="sort-catagory item-gap"
        ...
        <div class="sidebar-footer">
          <a href="#" class="btn apply" @click.prevent="reset()">Clear</a>
...
export default class ShopSortingSidebarMobile extends Vue {
  @Prop() filterData: any;
  @Prop() value!: any;
  ...
  get genders() {
    if (this.pageType == "generic") {
      ...
      return createCategoryTree(this.filterData.categories);
    } else {
      return null;
    }
  }
  ...
  mergedCategories(categories: any[]) {
    const data: any = {};
    for (let gidx = 0; gidx < categories.length; gidx++) {
      const gender = categories[gidx];
      if (gender.children) {
        for (let cidx = 0; cidx < gender.children.length; cidx++) {
          const category = gender.children[cidx];
          if (data[category.name] === undefined) {
            data[category.name] = { name: category.name, id: [category.id] };
          } else {
  ...
  get categories() {
    if (this.filterData.category != undefined) {
      return this.filterData.category.children.edges.map((v: any) => v.node);
    } else if (this.filterData.categories != undefined) {
      if (this.internalValue.gender !== null) {
        const cat = this.genders.find(
        ...
        return this.mergedCategories(this.genders);
      }
    } else {
      return [];
    }
